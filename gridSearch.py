...
model_dirs = {
    ...
    'STEP3_logs' : '%s/%s/h%d_lr%s_b%s_c%s_%s_%s_%s.csv' % (default_model_dir, 'STEP3_logs', params['STEP1']['n_hidden'], str(params['STEP1']['learning_rate']), str(params['STEP1']['reg_weight']), str(params['STEP1']['corruption_level']), params['STEP1']['model'], params['STEP4']['model'], params['experiment_type']),
...
if __name__ == '__main__':
    ...
    if params['experiment_type'] == 'baseline':
        ...
        dataset.unify_stockprices(dataset=dataset.baseline_original, brandcodes=params['STEP3']['brandcode'], dataset_type=params['experiment_type'], label_type=label_type, y_type=get_y_type(label_type))
    else:
        print 'start to load proposed dataset...'
        dataset = cPickle.load(open(model_dirs['STEP2']))
        print 'start to unify stockprice...'
        dataset.unify_stockprices(dataset=dataset.unified, brandcodes=params['STEP3']['brandcode'], dataset_type=params['experiment_type'], label_type=label_type, y_type=get_y_type(label_type))
