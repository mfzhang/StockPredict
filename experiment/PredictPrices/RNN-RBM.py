...
class RnnRbm(RBM):
    def __init__(self, n_visible=2, n_hidden=4, n_hidden_recurrent=3, lr=1.0):
        
        self.n_visible = n_visible
        self.n_hidden = n_hidden
        self.n_hidden_recurrent = n_hidden_recurrent

        self.W = sharedX(np.zeros([n_visible, n_hidden]), name='W', borrow=True)
        self.Wuh = sharedX(np.random.random([n_hidden_recurrent, n_hidden]), name='Wuh', borrow=True)
        self.Wuv = sharedX(np.random.random([n_hidden_recurrent, n_visible]), name='Wuv', borrow=True)
        self.Wvu = sharedX(np.random.random([n_visible, n_hidden_recurrent]), name='Wvu', borrow=True)
        self.Wuu = sharedX(np.random.random([n_hidden_recurrent, n_hidden_recurrent]), name='Wuu', borrow=True)
        self.bv = sharedX(np.zeros(n_visible), name='b_vt', borrow=True)
        self.bh = sharedX(np.zeros(n_hidden), name='b_ht', borrow=True)
        self.bu = sharedX(np.random.random(n_hidden_recurrent), name='b_ut', borrow=True)
        ...
        (v, v_sample, cost, monitor, params, updates_train, v_t,
         updates_generate) = self.build_rnnrbm(n_visible, n_hidden,
                                           n_hidden_recurrent)
    ...
    def build_rnnrbm(self, n_visible, n_hidden, n_hidden_recurrent):
        ...
        (u_t, bv_t, bh_t), updates_train = theano.scan(
            ...
            sequences=v, outputs_info=[u0, None, None], non_sequences=self.params)
        v_sample, cost, monitor, updates_rbm = self.build_rbm(v, self.W, bv_t[:], bh_t[:],
                                                         k=15)
        ...
        return (v, v_sample, cost, monitor, self.params, updates_train, v_t,
                updates_generate)
